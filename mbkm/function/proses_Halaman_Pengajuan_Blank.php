<?php
include 'koneksi.php'; // Menghubungkan file koneksi ke database

// Simulasi data pengguna
$nim = "4342411088";
$name = "Muhammad Ridho Syahputra";

// Simulasi fungsi render HTML dalam PHP
function renderSidebar() {
    return "<div class='sidebar'>
                <div class='logo'>
                    <img src='kampus_merdeka.png' alt='Logo'>
                </div>
                <nav>
                    <a href='#' class='dashboard'>
                        <span class='icon'><i class='fas fa-home-alt'></i></span>
                        <span class='text'>DASHBOARD</span>
                    </a>
                    <a href='#' class='pengajuan'>
                        <span class='icon'><i class='fas fa-file-alt'></i></span>
                        <span class='text'>PENGAJUAN</span>
                    </a>
                </nav>
            </div>";
}

function renderHeader($title) {
    global $name, $nim;
    return "<div class='header'>
                <h1>{$title}</h1>
                <div class='user-menu'>
                    <div class='profile-icon' onclick='toggleDropdown()'>
                        <img src='ikon_profil.png' alt='Profile Icon'>
                    </div>
                    <div class='dropdown' id='dropdownMenu'>
                        <span>{$name}<br>NIM: {$nim}</span>
                        <button><i class='fas fa-user'></i> Profile</button>
                        <button><i class='fas fa-key'></i> Change Password</button>
                        <button><i class='fas fa-sign-out-alt'></i> Logout</button>
                    </div>
                </div>
            </div>";
}

function renderTable() {
    return "<table id='example' class='display' style='width:100%'>
                <thead>
                    <tr>
                        <th>NIM</th>
                        <th>Nama</th>
                        <th>Program Studi</th>
                        <th>Tanggal Pengajuan</th>
                        <th>Status Pengajuan</th>
                        <th>Status Berkas</th>
                        <th>Detail</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
            </table>
            <script>
                new DataTable('#example');
            </script>";
}

function renderMessage($message) {
    return "<p><h3 class='text-center'><b>{$message}</b></h3></p>";
}

// Output halaman
echo "<!DOCTYPE html>
<html lang='en'>
<head>
    <link rel='stylesheet' href='style.css'>
    <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css'>
    <title>Halaman Pengajuan</title>
    <link rel='stylesheet' href='https://cdn.datatables.net/2.1.8/css/dataTables.dataTables.css'>
    <script src='https://code.jquery.com/jquery-3.7.1.js'></script>
    <script src='https://cdn.datatables.net/2.1.8/js/dataTables.js'></script>
    <link href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css' rel='stylesheet'>
    <link href='https://getbootstrap.com/docs/5.3/assets/css/docs.css' rel='stylesheet'>
    <script src='https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js'></script>
    <link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css'>
</head>
<body>";

echo renderSidebar();
echo "<div class='content'>";
echo renderHeader("Pengajuan");
echo renderHeader("Riwayat Pengajuan");
echo "<div><button type='button' class='btn btn-primary'>Tambah Pengajuan</button></div>";
echo renderTable();
echo renderMessage("ANDA BELUM MELAKUKAN PENGAJUAN...");
echo "</div>
</body>
</html>";
?>
